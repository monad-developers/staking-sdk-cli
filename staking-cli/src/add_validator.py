...
from src.helpers import count_zeros, amount_prompt, key_prompt, address_prompt, wei, is_valid_secp256k1_private_key, is_valid_bls_private_key, is_valid_address, is_valid_amount
...
def register_validator(config: dict):
    ...
    try:
        keygen = KeyGenerator.from_keys(secp_private_key=secp_privkey_hex, bls_private_key=bls_privkey_hex)
        secp_pubkey = keygen.pub_secp_key()
        bls_pubkey = keygen.pub_bls_key()
    except Exception as e:
        log.error(f"Key derivation failed: Your keys fall in the correct curve order but are incompatible with the cryptographic scheme: {e}")
        return
    ...
    amount = wei(amount_prompt(config, method="add_validator", description="to stake to validator (min 1 million MON to register validator)"))
    auth_address = address_prompt(config, address_description="Authorized Address", default=funded_address)
    ...
    table.add_row("[cyan]Input [bold red]SECP Private Key[/bold red] (Hex String):[/]", f"[green]{secp_privkey_hex}[/]")
    table.add_row("[cyan]Input [bold red]BLS Private Key[/bold red] (Hex String):[/]", f"[green]{bls_privkey_hex}[/]")
    table.add_row("[cyan]Amount to [bold red]stake[/] while adding validator:[/]", f"[green]{amount} (contains {count_zeros(amount)} zeros)[/]")
    table.add_row("[cyan][bold red]Authorized Address[/] for delegation:[/]", f"[green]{auth_address}[/]")
    table.add_row("[cyan][bold red]Funded Address's Private Key[/] to use for tx:[/]", f"[green]{funded_private_key}[/]")
    ...
    # send tx
    try:
        tx_hash = send_transaction(w3, funded_private_key, contract_address, add_validator_call_data, chain_id, amount, gas_limit=2_000_000)
...
def register_validator_cli(config: dict, secp_privkey: str, bls_privkey: str, auth_address: str, amount: int):
    log = init_logging(config["log_level"].upper())
    ...
    try:
        ...
        if not is_valid_amount(amount, register=True):
            log.error("The amount enter was invalid for validator registration, should be integer greater than or equal to 1,000,000 MON")
            return
    except Exception as e:
    ...
    # Generate the pubkeys and address
    try:
        keygen = KeyGenerator.from_keys(secp_private_key=secp_privkey, bls_private_key=bls_privkey)
        secp_pubkey = keygen.pub_secp_key()
        bls_pubkey = keygen.pub_bls_key()
    ...
    # send tx
    try:
        tx_hash = send_transaction(w3, funded_private_key, contract_address, add_validator_call_data, chain_id, amount_wei, gas_limit=2_000_000)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
    except Exception as e:
...
def get_validator_registration_event(config, receipt):
    ...
    w3 = Web3(Web3.HTTPProvider(rpc_url))
    if not receipt:
        receipt = w3.eth.wait_for_transaction_receipt("768f8911c7db93e5910c0f92d7cd71807a9b58d24de5e95deda8f219ca541e21")
